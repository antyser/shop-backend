...
def create_llm_client(provider="openai"):
    if provider == "openai":
        ...
        return AzureOpenAI(api_key=api_key, api_version="2024-08-01-preview", azure_endpoint="https://msopenai.openai.azure.com")
...
def query_llm(prompt: str, client=None, model=None, provider="openai", image_path: str | None = None) -> str | None:
    ...
    try:
        ...
        if provider in ["openai", "local", "deepseek", "azure"]:
            ...
            if image_path:
                encoded_image, mime_type = encode_image_file(image_path)
                messages[0]["content"].append({"type": "image", "source": {"type": "base64", "media_type": mime_type, "data": encoded_image}})
...
def main():
    ...
    parser.add_argument("--provider", choices=["gemini"], default="gemini", help="The API provider to use")
